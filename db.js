...
async function upsertUser(user) {
  const query = `
    ...
    ON CONFLICT (user_id) DO UPDATE SET
      ...
      followers_count = GREATEST(users.followers_count, EXCLUDED.followers_count), -- Keep highest seen
...
async function upsertRoom(room) {
  const query = `
    ...
    ON CONFLICT (room_id) DO UPDATE SET
      ...
      total_unique_visitors = rooms.total_unique_visitors + 1; -- Simple increment example
  `;
  const values = [
...
async function syncRoomSessions(roomId, usersInRoom) {
    // 1. Close sessions for users who are NO LONGER in the list
    // We assume if they aren't in 'usersInRoom', they left.
    if (usersInRoom.length > 0) {
        const currentUserIds = usersInRoom.map(u => `'${u.user_id}'`).join(',');
        
        // Close sessions where user is NOT in the new list but WAS in this room
        await pool.query(`
            UPDATE sessions 
        ...
        // Room is empty? Close ALL active sessions for this room
        await pool.query(`
            UPDATE sessions 
            SET left_at = NOW(), is_active = false 
    ...
    // 2. Open new sessions for users who ARE in the list (if not already active)
    for (const user of usersInRoom) {
        // Ensure user exists first
        await upsertUser(user);

        // Try to insert a new active session if one doesn't exist
        // Note: This logic assumes one user one room at a time.
        const sessionQuery = `
